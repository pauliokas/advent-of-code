...
These caves seem to be [lava tubes](https://en.wikipedia.org/wiki/Lava%5Ftube). Parts are even still volcanically
active; small hydrothermal vents release smoke into the caves that slowly settles like rain.
...
Your first goal is to find the _low points_ \- the locations that are lower than any of its adjacent locations. Most
...
In the above example, there are _four_ low points, all highlighted: two are in the first row (a `1` and a `0`), one is
...
The _risk level_ of a low point is _1 plus its height_. In the above example, the risk levels of the low points are `2`,
`1`, `6`, and `6`. The sum of the risk levels of all low points in the heightmap is therefore `_15_`.
...
A _basin_ is all locations that eventually flow downward to a single low point. Therefore, every low point has a basin,
...
The _size_ of a basin is the number of locations within the basin, including the low point. The example above has four
basins.
