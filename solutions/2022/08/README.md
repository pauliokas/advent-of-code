...
for a [tree house](https://en.wikipedia.org/wiki/Tree%5Fhouse).
...
With 16 trees visible on the edge and another 5 visible in the interior, a total of `_21_` trees are visible in this
arrangement.
...
- Looking down, its view is blocked eventually; it can see `_2_` trees (one of height `3`, then the tree of height `5`
...
tree, this is `_4_` (found by multiplying `1 * 1 * 2 * 2`).
