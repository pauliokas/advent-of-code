...
Each rucksack has two large _compartments_. All items of a given type are meant to go into exactly one of the two
...
(`L`), 42 (`P`), 22 (`v`), 20 (`t`), and 19 (`s`); the sum of these is `_157_`.
...
the first group and 52 (`Z`) for the second group. The sum of these is `_70_`.
